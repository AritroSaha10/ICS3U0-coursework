#-----------------------------------------------------------------------------
# Name:        Logging Practice
# Purpose:     Practice proper logging usage
#
# Author:      Aritro Saha
# Created:     21-Apr-2022
# Updated:     21-Apr-2022
...
dollarPerLitre = 1.903 # Canadian gas price, taken from this https://www.globalpetrolprices.com/gasoline_prices/
...
def milesToKm(distance):
  ...
  logging.info("Function milesToKm() is beginning with distance = " + str(distance))

  # Ensure types match
  if not isinstance(distance, (int, float)):
    logging.error("Distance parameter is incorrect type (not int/float).")
    raise TypeError("Distance parameter is not int/float.")
  
  # Distance cannot be smaller than 0
  if distance < 0:
    logging.error("Distance parameter is negative.")
    raise ValueError("Distance parameter is negative.")

  logging.debug("Parameters are OK, proceeding with calculation...")

  # 1.60934 is the unit conversion from miles to kilometres
  distKm = 1.60934 * distance

  logging.info(f"Distance in km calculated to be: {distKm}, returning rounded value.")

  return round(distKm, 2)
...
def saleCalc(price, discount):
  ...
  logging.info(f"Function saleCalc() is beginning with price = {price} and discount = {discount}")
  
  # Ensure types match
  if not isinstance(price, (int, float)):
    logging.error("Price parameter was wrong type (not int, float)")
    raise TypeError("Price parameter is not int or float.")

  if not isinstance(discount, (int, float)):
    logging.error("Discount parameter was wrong type (not int, float)")
    raise TypeError("Discount parameter is not int or float.")

  # Raise error if parameters do not follow restrictions
  if price < 0 or discount < 0:
    logging.error("Price or discount parameter was negative.")
    raise ValueError("Price or discount parameter is negative.")

  if discount >= 100:
    logging.error("Discount parameter is larger than 100%.")
  ...
  logging.info(f"Discounted price calculated to be: {discountedPrice}, returning rounded value.")
  
  # Return the discounted price rounded to 2 decimal places
  return round(discountedPrice, 2)

logging.info("Testing functions using assertions...")
...
assert milesToKm(1923) == 3094.76, "1923 miles (rounded to 2 decimal points) is 3094.76km"
# assert milesToKm(-1) == "Error.", "Impossible to convert negative distances"

# Test the saleCalc to ensure it works as intended
assert saleCalc(100, 50) == 50, "50% off of $100 results in a final price of $50"
assert saleCalc(20, 23) == 15.4, "23% off of $20 results in a final price of $15.4"
...
stores = {
  ...
  "Costco": {
    ...
    "food": {
      ...
      "Rotisserie Chicken with 12% Discount": saleCalc(30, 0.12), # in dollars
      "Celery": 0.7, # in dollars
...
logging.debug("Gathering user's store choice.")

# Get the store chosen by making the user type in the index from before
try:
  chosenStoreIdx = int(input("Type the number here: ")) - 1
except Exception as e:
  logging.error(f"Error when gathering store index from user: {str(e)}")
  print("Something went wrong:", str(e))
else:
  logging.debug("Store index successfully gathered, proceeding...")
  logging.info(f"User chose store index {chosenStoreIdx}, converting to name...")
  try:
    chosenStore = list(stores.keys())[chosenStoreIdx]
    chosenStoreData = stores[chosenStore]
  except Exception as e:
    print("Something went wrong:", e)
    logging.error(f"Error when getting store name from store index: {str(e)}")
  else:
    logging.info(f"User chose store: {chosenStore}")
    logging.info("Store data and name stored")
    # Separator line
    print()
    
    # Calculate driving distance and price (rounded to 2 decimal places)
    try:
      logging.debug("Converting distance to store to kilometres using milesToKm()...")
      kmToDrive = milesToKm(chosenStoreData["distance"])
    except Exception as e:
      print("Something went wrong:", str(e))
      logging.error(f"Error when converting miles to kilometres using milesToKm: {str(e)}")
    else:
      logging.info(f"Kilometres to drive to store (one way): {kmToDrive}")
      logging.debug("Using converted kilometres to determine litres of gas and amount of gas money needed for one way trip...")
      gasNeeded = round(litresPerKm * kmToDrive, 2)
      gasMoney = round(gasNeeded * dollarPerLitre, 2)
      logging.info(f"Gas needed (in L): {gasNeeded}")
      logging.info(f"Gas money needed (in $): {gasMoney}")
      
      # Display driving info
      print(f"Driving to {chosenStore} will take {chosenStoreData['distance']} miles, or {kmToDrive} kilometres")
      print(f"You have a Honda Civic, so you will be using {gasNeeded}L of fuel, which equates to ${gasMoney} in gas money.")
      ...
      logging.debug("Asking user for what they would like to buy from the store...")
      
      # Allow user to input things to buy as "1 2 3 1"
      thingsToBuyRaw = input(f"What would you like to buy?\nInput multiple space-separated integers representing what you'd like to buy (ex. 1 1 1 1 to buy four \'{allFoods[0]}\'): ")
  
      try:
        logging.debug("Converting each index from user into a food name...")
        
        # Turn raw input into each food name
        thingsToBuyCleaned = [allFoods[int(num) - 1] for num in thingsToBuyRaw.split(" ")]
        
        logging.debug("Conversion successful, creating frequency chart from names...")
        ...
        logging.debug("Frequency chart generation successful, calculating subtotal of all items...")
        ...
        for item, freq in thingsToBuyFreq.items():
          ...
          logging.info(f"Number of '{item}': {freq}")
          logging.info(f"Total price for {freq} of '{item}': {freq * chosenStoreData['food'][item]}")
          
        logging.info(f"Total subtotal: {grocerySubtotal}")
        logging.debug("Subtotal successfully calculated")
      except Exception as e:
        logging.error("Error when converting user input of what to buy into grocery subtotal")
        print("Something went wrong:", str(e))
      else:
        logging.info(f"Adding 15% discount to subtotal using saleCalc, with args price: {grocerySubtotal} and discount: 15")
        
        # Add tax and 15% discount, and round to 2 decimal places
        groceryTotal = round(saleCalc(grocerySubtotal, 15) * 1.13, 2)
        logging.info(f"Total money spent at the grocery (w/ tax + 15% discount): {groceryTotal}")
        ...
        try:
          logging.debug("Asking for extra driven miles from user...")
          
          # Ask for any more miles they may have driven
          extraDrivenMiles = float(input("Input any more driving (in miles) that you may have done while on the trip: "))
          logging.info(f"User drove {extraDrivenMiles} extra miles during their trip")
        except Exception as e:
          logging.error("Error when gathering input regarding extra driven miles")
          print("Something went wrong:", e)
        else:
          # Calculate driving distance in KM and price
          try:
            logging.info(f"Converting the {extraDrivenMiles} extra driven miles to kilometres using milesToKm with args distance: {extraDrivenMiles}")
            
            extraKmDrive = milesToKm(extraDrivenMiles)
            
            logging.info(f"Extra driven miles in kilometres (as converted by milesToKm): {extraKmDrive}")
          except Exception as e:
            logging.error(f"Error when converting extra driven miles to kilometres")
            print("Something went wrong:", e)
          else:
            logging.debug("Calculating the extra gas in L and gas money needed to travel the distance...")
            
            gasNeededExtra = round(litresPerKm * extraKmDrive, 2)
            gasMoneyExtra = round(gasNeededExtra * dollarPerLitre, 2)
            
            logging.info(f"Extra gas needed (in L, rounded to 2 decimal points): {gasNeededExtra}")
            logging.info(f"Extra gas money needed (in $, rounded to 2 decimal points): {gasMoneyExtra}")

            totalSpent = round(groceryTotal + gasMoney * 2 + gasMoneyExtra, 2)
            
            # Display all money spent
            print(f"Your trip is done! Total amount spent (incl. gas): ${totalSpent}")
            logging.info(f"Total amount spend (incl. gas): {totalSpent}")
